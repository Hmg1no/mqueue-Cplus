 ...
 ** File:    Swarm.cpp
 ** Project: Fall 2024 - Proj3
 ** Author:  Hazael Magino
 ** Date:    10/30/2022
 ** This file contains the proper implementations for mqueue.h
 **********************************************************/
#ifndef MQUEUE_H
#define MQUEUE_H
...
private:
  ...
  // it takes a value between 1 and 50, at any time the plant only accepts 50
  // orders at max therefore this value cannot go beyond 50 a value of 1 means
  // this was the first customer who placed the order a value of 50 means this
  // was the 50th customer who placed the order
  ...
  int m_material;   // material availability, (percentage) = 100(higher) -
                    // 0(lower)
  int m_importance; // customer importance (percentage) = 100(higher) - 1(lower)
  int m_workForce;  // work force availability, (person count) = 200(higher) -
                    // 100(lower)
  int m_quantity;   // quantity of the order, (unit count) = 10000(higher) -
                    // 1000(lower)
};
...
private:
   ...
   ******************************************/
  void deleteNodes(Node *aNode); // helper function to delete nodes
  Node *mergeRecurse(Node *newTree,
                     Node *rhsTree); // helper function to iterate subtree
                                     // during merge operation
  void swap(Node *&left,
            Node *&right); // helper function to swap children nodes of a node
  void copyTree(Node *&newNode,
                const Node *oldNode);  // helper function for copying heap
  void printHeap(Node *newNode) const; // helper function for printing output
  Node *rebuild(Node *oldHeap);        // helper function to reconstruct a heap
                                       // depending on new heaptype or structure
  void checkNPL(
      Node *node); // helper function to readjust npl values of a leftist heap
};
